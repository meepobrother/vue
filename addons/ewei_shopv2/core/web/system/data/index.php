			...
			if (is_array($_GPC['shop'])) 
			{
				foreach ($_GPC['shop'] as $data ) 
				{
					...
					else if ($data == 'membercredit1') 
					{
						if ($wechatid != -1) 
						{
							$members = pdo_fetchall('select id,uid,openid,uniacid from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid ', array(':uniacid' => $wechatid));
						}
						else 
						{
							$members = pdo_fetchall('select id,uid,openid,uniacid from ' . tablename('ewei_shop_member'));
						}
						foreach ($members as $m ) 
						{
							...
							if (empty($mm['uid'])) 
							{
								pdo_update('ewei_shop_member', array('credit1' => 0), array('id' => $mm['id']));
							}
							else 
							{
					...
					else if ($data == 'membercredit2') 
					{
						...
						foreach ($members as $m ) 
						{
							...
							if (empty($mm['uid'])) 
							{
								pdo_update('ewei_shop_member', array('credit2' => 0), array('id' => $mm['id']));
							}
							else 
							{
