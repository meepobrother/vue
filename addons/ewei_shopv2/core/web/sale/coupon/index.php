...
class Index_EweiShopV2Page extends ComWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			//show_json(1, array('url' => 'http://fx.abfau.cn/web/index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=sale.coupon.add'));
		}
			...
			foreach ($ds as &$d ) 
			{
				...
				else if ($live) 
				{
					...
					else if ($d['backtype'] == 2) 
					{
						...
						else if (!(empty($d['backcredit'])) && (0 < $d['backcredit'])) 
						{
							$d['value_text'] = '积分';
							$d['value_total'] = $d['backcredit'];
						}
						else if (!(empty($d['backredpack'])) && (0 < $d['backredpack'])) 
						{
							$d['value_text'] = '0';
