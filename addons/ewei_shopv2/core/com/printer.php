...
class Printer_EweiShopV2ComModel extends ComModel 
{
	...
	public function style_list_code($str, $code = 0, $type = 0) 
	{
		if (($type == '0') || ($type == '1')) 
		{
			switch ($code) 
			{
				case self::CONTENT_BOLD: $res = '<B>' . $str . '</B><BR>';
				break;
				case self::CONTENT_CENTER: $res = '<C>' . trim($str) . '</C>';
				break;
				case self::CONTENT_CENTER_BOLD: $res = '<CB>' . trim($str) . '</CB><BR>';
				break;
				case self::CONTENT_CODE: $res = '<CODE>' . $str . '<BR>';
				break;
				case self::CONTENT_QRCODE: $res = '<QR>' . $str . '</QR>';
				break;
				default: $res = $str . '<BR>';
				break;
			}
		...
		else if ($type == '2') 
		{
			switch ($code) 
			{
				case self::CONTENT_BOLD: $res = '@@2' . $str . "\n";
				break;
				case self::CONTENT_CENTER: $res = '<center>' . trim($str) . '</center>';
				break;
				case self::CONTENT_CENTER_BOLD: $res = '<center>@@2' . trim($str) . '</center>' . "\n";
				break;
				case self::CONTENT_CODE: $res = '<b>' . $str . '</b>';
				break;
				case self::CONTENT_QRCODE: $res = '<q>' . $str . '</q>';
				break;
				default: $res = $str . "\n";
				break;
			}
		...
		else if ($type == '3') 
		{
			switch ($code) 
			{
				case self::CONTENT_BOLD: $res = '^H2' . $str . "\n";
				break;
				case self::CONTENT_CENTER_BOLD: $res = '^B2' . trim($str) . "\n";
				break;
				case self::CONTENT_CODE: $qrlength = chr(strlen($str));
				$res = '^P' . $qrlength . $str . "\n";
				break;
				case self::CONTENT_QRCODE: $qrlength = chr(strlen($str));
				$res = '^Q' . $qrlength . $str . "\n";
				break;
				default: $res = $str . "\n";
				break;
		...
		else if ($type == '4') 
		{
			switch ($code) 
			{
				case self::CONTENT_BOLD: $res = '<FB>' . $str . '</FB>' . "\n";
				break;
				case self::CONTENT_CENTER: $res = '<center>' . trim($str) . '</center>';
				break;
				case self::CONTENT_CENTER_BOLD: $res = '<center><FB>' . trim($str) . '</FB></center>' . "\n";
				break;
				case self::CONTENT_CODE: $res = '<BR>' . $str . '</BR>';
				break;
				case self::CONTENT_QRCODE: $res = '<QR>' . $str . '</QR>';
				break;
				default: $res = $str . "\n";
	...
	public function printer($params, $templateid, $printerid, $type = 0) 
	{
			...
			foreach ($print_data as $key => &$value ) 
			{
				if (($key == 'key') && !(empty($print_data['key']))) 
				{
					...
					foreach ($print_data['key'] as $ke => $val ) 
					{
						...
						else if (($type == 0) && strexists($val, '[商品价格详情]')) 
						{
		...
		switch ($printer['type']) 
		{
			case self::PRINTRT_FEIE: $res = $this->printerFeie();
			break;
			case self::PRINTRT_YILIANYUN: $res = $this->printerYilianyun();
			break;
			case self::PRINTRT_YILIANYUN_NEW: $res = $this->printerYilianyunNew();
			break;
			case self::PRINTRT_365_S1: $res = $this->printer365s1();
			break;
			case self::PRINTRT_FEIE_NEW: $res = $this->printerFeieNew();
			break;
			default: $res = $this->printer365();
			break;
		}
		return $res;
	}
