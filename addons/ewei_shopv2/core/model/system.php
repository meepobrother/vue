...
class System_EweiShopV2Model
{
	...
	protected function initRightMenu($routes)
	{
		...
		if ($this->merch) {
			...
			if ($routes[0] == 'system') {
				$return_arr['menu_items'][] = array('text' => '返回商城', 'href' => webUrl());
			}
			else {
				$return_arr['menu_items'][] = array('text' => '切换公众号', 'href' => webUrl('sysset/account'));
				...
				if ($this->cv('perm')) {
					...
					$return_arr['menu_items'][] = array('text' => '权限管理', 'href' => webUrl('perm'));
				}
				...
				$return_arr['menu_items'][] = array('text' => '返回系统', 'href' => './index.php?c=account&a=display&', 'blank' => true);
